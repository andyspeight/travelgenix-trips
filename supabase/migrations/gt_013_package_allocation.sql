-- =============================================================================
--  gt_013_package_allocation.sql · gt_hold_places enforces per-package caps
-- =============================================================================
--  Additive over gt_012 (promo). When the chosen package sets its own capacity,
--  the hold counts what that package has taken on this departure (under the same
--  departure-row FOR UPDATE lock, so it cannot oversell) and refuses with
--  'package_full' if the party would go over. A package with a null capacity is
--  unaffected. Same 9-arg signature, so create-or-replace keeps the grant.
--  Live-verified: a 2-cap package fills at 2, a third booking on it returns
